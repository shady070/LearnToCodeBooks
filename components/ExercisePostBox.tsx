...
const reqUrl = 'https://freeresources.learntocodebooks.com/wp-json/wp/v2/exercises?acf_format=standard&_fields=acf,id';

const ExercisePostBox = () => {
        ...
        recentPosts.map((post, index) => (
          <div className="hover:drop-shadow-2xl" key={index}>
            ...
            <div className='w-full md:w-[330px] h-[111px] rounded-b-lg px-[31px] pt-[8px] bg-[#1B222C] text-white mb-[30px] md:mb-[0px] '>
              <Link href={`/exercisedetails/${post.id}`} className='text-[20px] font-medium pb-[11px] text-white cursor-pointer hover:text-[#FB8B01] active:text-[#FB7712]'>{post.acf.title}</Link>
...
export default ExercisePostBox;
